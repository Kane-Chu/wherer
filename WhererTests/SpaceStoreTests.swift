...
final class SpaceStoreTests: XCTestCase {
    ...
    func testDeleteSpaceCleansUpPhotoFiles() throws {
        ...
        // Legacy photoFilename file (still cleaned up by deleteSpace)
        ...
        // ItemPhoto with imageData (no disk file needed)
        let itemPhoto = ItemPhoto(context: context)
        itemPhoto.id = UUID()
        itemPhoto.filename = "\(itemPhoto.wrappedId.uuidString).jpg"
        itemPhoto.imageData = imageData
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: photoURL.path), "legacy照片文件应被清理")
    }
}
